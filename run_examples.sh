...
export OMP_NUM_THREADS=1; ./lda_scvb0 -d data_uci/docword.nips.txt -v data_uci/vocab.nips.txt -s 10 -k 100 --doctopicfile doctopic.txt --topicwordfile topics.txt --topicwordfile2 topics.vocab.txt 

# 2. as 1., yet using four threads. Note that reported time is cpu time, therefore divide by 2-3.5 to have better idea
#export OMP_NUM_THREADS=4; ./lda_scvb0 -d data_uci/docword.nips.txt -v data_uci/vocab.nips.txt -s 10 -k 100 --doctopicfile doctopic.txt --topicwordfile topics.txt --topicwordfile2 topics.vocab.txt 
...
#export OMP_NUM_THREADS=4; cat data_uci/docword.nips.txt | python get_docword2oneline.py | ./lda_scvb0 -p -d data_uci/docword.nips.txt -v data_uci/vocab.nips.txt -s 10 -k 100 --doctopicfile doctopic.txt --topicwordfile topics.txt --topicwordfile2 topics.vocab.txt > query.out 
